...
def train_one_epoch(model, train_dataloader, optimizer, scheduler, device, loss_fn, experiment=None, fold_num=0, gradient_accumulation_steps=1, is_binary_classification=False, is_regression=False, is_coral=False):
    model.train()
    ...
    for batch in p_bar:
        ...
        if is_binary_classification: 
            ...
            # For regression, predict continuous values and use MSE with target labels
            loss = loss_fn(outputs.squeeze(-1), labels.float())
        elif is_coral:
            # For CORAL, create cumulative targets and use MSE
            coral_targets = create_coral_targets(labels, n_classes + 1)  # +1 because n_classes is K-1 for CORAL
            print(f"coral_targets: {coral_targets}")
            coral_outputs = torch.sigmoid(outputs)  # Apply sigmoid to get probabilities
            print(f"coral_outputs: {coral_outputs}")
            loss = loss_fn(coral_outputs, coral_targets)
        else: 
...
def eval_one_epoch(model, eval_dataloader, device, loss_fn, experiment=None, is_binary_classification=False, is_regression=False, is_coral=False):
    model.eval()
    ...
    with torch.no_grad():
        for batch in p_bar:
            ...
            if is_binary_classification: 
                ...
                # For regression, round the continuous output to nearest integer
                predictions.extend(torch.round(outputs.squeeze(-1)).cpu().numpy().astype(int))
            elif is_coral:
                # For CORAL, use the CORAL prediction function
                coral_preds = coral_predict(outputs)
                predictions.extend(coral_preds.cpu().numpy().astype(int))
            else: 
...
def compute_metrics(reference, predictions, predictions_prob, verbose=False, is_binary_classification=False):
    ...
    # print reference and prediction
    print("Reference:", reference[:10])
...
def fix_updrs_speech_hy_labels(df):
    df["UPDRS-speech"] = df["UPDRS-speech"].fillna(0)
    df["UPDRS-speech"] = df["UPDRS-speech"].astype(int)
    df["H/Y"] = df["H/Y"].fillna(0)
    df["H/Y"] = df["H/Y"].astype(str).str.replace(',', '.').astype(float)
    return df

def get_dataloaders(train_path, test_path, class_mapping, config, is_regression=False, is_coral=False, pd_only=False):
        
    df_test = pd.read_csv(test_path)
    # remove the rows containing "words" in the audio_path
    df_test = fix_updrs_speech_hy_labels(df_test)
    
    # Apply PD_only filter if specified
    if pd_only:
        print("Applying PD_only filter - removing HC subjects")
        print(f"Original test set size: {len(df_test)}")
        df_test = df_test[df_test['status'] != 'hc']
        print(f"Filtered test set size: {len(df_test)}")
    
    test_paths = df_test.audio_path.values.tolist()
    test_labels = df_test[config.training.label_key].values.tolist()
    
    df_train = pd.read_csv(train_path)
    df_train = fix_updrs_speech_hy_labels(df_train)
    
    # Apply PD_only filter if specified
    if pd_only:
        print(f"Original train set size: {len(df_train)}")
        df_train = df_train[df_train['status'] != 'hc']
        print(f"Filtered train set size: {len(df_train)}")
    ...
    # Apply class mapping to get the final training labels
    mapped_t_labels = [class_mapping[label] for label in t_labels]
    
    # set model.num_classes according to the training mode
    if is_regression:
        # For regression, use single output
        config.model.num_classes = 1
    elif is_coral:
        # For CORAL, use K-1 outputs for K classes
        config.model.num_classes = len(set(mapped_t_labels)) - 1
    else:
        # For classification, use number of classes
        config.model.num_classes = len(set(mapped_t_labels))
...
if __name__ == "__main__":
    ...
    if config.training.label_key == "status":
        ...
        print(f"Class mapping: {class_mapping}")
    elif config.training.label_key == "H/Y":
        is_binary_classification = False
        if config.training.hy_bin is True:
            class_mapping = {0: 0, 1: 1, 1.5: 1, 2: 1, 2.5: 2, 3: 2, 4: 3, 5: 3}
        else:
            class_mapping = {0: 0, 1: 1, 1.5: 2, 2: 2, 2.5: 3, 3: 3, 4: 4, 5: 5}
        print(f"Class mapping: {class_mapping}")

    # Determine the training mode
    is_regression = (not is_binary_classification and 
                    hasattr(config.training, 'multi_loss') and 
                    config.training.multi_loss == "regression")
    
    is_coral = (not is_binary_classification and 
               hasattr(config.training, 'multi_loss') and 
               config.training.multi_loss == "coral")
    
    # Check if PD_only mode is enabled
    pd_only = (hasattr(config.training, 'pd_only') and 
               config.training.pd_only == True)
    
    if pd_only:
        print("PD_only mode enabled - will filter out HC subjects")
    ...
    for test_fold in range(1, config.data.num_folds+1):
        ...
        train_dl, val_dl, test_dl = get_dataloaders(train_path, test_path, class_mapping, config, is_regression, is_coral, pd_only)
        ...
        if is_binary_classification: 
            ...
            if config.training.multi_loss == "cross_entropy": 
                loss_fn = torch.nn.CrossEntropyLoss()
            elif config.training.multi_loss == "regression":
                loss_fn = torch.nn.MSELoss()
            elif config.training.multi_loss == "coral":
                loss_fn = torch.nn.MSELoss()
            else:
                raise ValueError(f"Unsupported multi_loss type: {config.training.multi_loss}")

        print(f"Loss type: {config.training.multi_loss}, function: {loss_fn}")
        ...
        for epoch in range(config.training.num_epochs):
            ...
            if config.training.validation.active:
                ...
                # save the best model
                if f1 > best_metric:
                    print(f"Found a better model with f1: {f1:.3f} - previous best: {best_metric:.3f}")
                    best_metric = f1
